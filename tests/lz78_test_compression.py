...
def test(input):
	...
	compressed_file = os.path.join(results, os.path.basename(input) + ".lz78") #adds the results to the test_results folder, giving them clear filetypes for which is which
	decompressed_file = os.path.join(results, os.path.basename(input) + ".lz78.txt")
