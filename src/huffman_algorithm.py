...
def gen_codes(node, path="", store_codes=None): #generating the codes for the algorithm to use in order to decode it
    if store_codes=None:
        store_codes={}
    if node.val is not None:
        store_codes[node.val]=path
    else:
        gen_codes(node.left, path + "0", store_codes) #generates the codes, adds 0 for each left node and 1 for each right node
        gen_codes(node.right, path + "1", store_codes)
    return store_codes
